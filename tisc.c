...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < lines; i++) {
		...
		if (strstr(prog[i], "#")){}
			...
			if (check[0] != '\0') {
				printf("tisc: invalid label at line %d: label can not have instruction following label declaration\n", i);
				fclose(fl);
				for (int j = 0; j <= i; j++) {
					free(prog[j]);
				}
...
int parse(char* opcode, char* arg) {
	...
	if (!strcmp(opcode, "HALT")) {
		...
		if (arg[0] == '\0') {
			...
			printf("tisc: invalid argument count at line %d: instruction takes 0 arguments\n", pointer);
			return 1;
		}
...
int validLabel(char* label, int l) {
	if(validOpcode(label)){
		printf("tisc: invalid label at line %d: label name can not be an opcode\n",l);
		return 0;
	}
	int i;
	for (i = 0; label[i] != '\0'; i++) {
